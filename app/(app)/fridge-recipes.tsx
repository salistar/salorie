...
export default function FridgeRecipesScreen() {
  ...
  const run = async (fromCamera: boolean) => {
    try {
      ...
      setResult(`Analyse impossible (${e?.message || 'erreur'}). Réessaie avec une photo plus claire.`);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.body}>
        <View style={styles.head}><Refrigerator size={24} color={GREEN} /><Text style={styles.title}>Frigo → recettes</Text></View>
        <Text style={styles.sub}>Photographie ton frigo/placard — l'IA identifie les ingrédients et propose des recettes.</Text>

        <View style={styles.btnRow}>
          <TouchableOpacity style={[styles.btn, styles.btnPrimary]} onPress={() => run(true)} disabled={loading}>
            <Camera size={20} color="#fff" /><Text style={styles.btnPrimaryTxt}>Caméra</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.btn, styles.btnGhost]} onPress={() => run(false)} disabled={loading}>
            <ImageIcon size={20} color={GREEN} /><Text style={styles.btnGhostTxt}>Galerie</Text>
          </TouchableOpacity>
        </View>

        {uri && <Image source={{ uri }} style={styles.preview} resizeMode="cover" />}
        {loading && <View style={styles.center}><ActivityIndicator color={GREEN} /><Text style={styles.loadingTxt}>Analyse de la photo…</Text></View>}
        {!!result && <View style={styles.resultCard}><Text style={styles.resultTxt}>{result}</Text></View>}
        {!uri && !loading && <Text style={styles.hint}>📸 Astuce : ouvre la porte du frigo, bonne lumière, ingrédients visibles.</Text>}
      </ScrollView>
    </SafeAreaView>
  );
}
