...
export default function ShoppingListScreen() {
  ...
  const add = () => { const t = text.trim(); if (!t) return; persist([{ id: String(Date.now()), name: t, done: false }, ...items]); setText(''); };
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.body} keyboardShouldPersistTaps="handled">
        <View style={styles.head}><ShoppingCart size={24} color={GREEN} /><Text style={styles.title}>Liste de courses</Text></View>
        <Text style={styles.sub}>{items.length ? `${left} article(s) à acheter` : 'Ajoute ce qu\'il te faut.'}</Text>

        <View style={styles.addRow}>
          <TextInput style={styles.input} placeholder="Ajouter un article…" value={text} onChangeText={setText} onSubmitEditing={add} returnKeyType="done" />
          <TouchableOpacity style={styles.addBtn} onPress={add}><Plus size={22} color="#fff" /></TouchableOpacity>
        </View>

        {items.length === 0 ? <Text style={styles.empty}>Liste vide.</Text> : items.map((i) => (
          <View key={i.id} style={styles.item}>
            <TouchableOpacity style={[styles.check, i.done && styles.checkDone]} onPress={() => toggle(i.id)}>
              {i.done && <Check size={16} color="#fff" />}
            </TouchableOpacity>
            <Text style={[styles.itemName, i.done && styles.itemDone]}>{i.name}</Text>
            <TouchableOpacity onPress={() => remove(i.id)} hitSlop={8}><Trash2 size={18} color="#CBD5E1" /></TouchableOpacity>
          </View>
        ))}

        {items.some((i) => i.done) && <TouchableOpacity style={styles.clearBtn} onPress={clearDone}><Text style={styles.clearTxt}>Retirer les articles cochés</Text></TouchableOpacity>}
      </ScrollView>
    </SafeAreaView>
  );
}
